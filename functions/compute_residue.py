...
def compute_residue(df):
    ...
    df = df.assign(
        **{
            "Load serviced by renewables [kW]": df[
                ["Load profile [kW]", "Renewables profile [kW]"]
            ].min(axis=1),
    ...
    residue = pd.DataFrame().assign(
        **{
            "Residual load profile [kW]": df["Load profile [kW]"]
            - df["Load serviced by renewables [kW]"],
            "Residual renewables profile [kW]": df["Renewables profile [kW]"]
            - df["Load serviced by renewables [kW]"],
        }
    )

    for idx in df.index:
        if residue.loc[idx, "Residual load profile [kW]"] > 0.0:
            service_residual_load(df, residue, idx)

        elif residue.loc[idx, "Residual renewables profile [kW]"] > 0.0:
            distribute_residual_energy(df, residue, idx)
